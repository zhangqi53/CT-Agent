...
for (const tool of TOOL_REGISTRY) {
    ...
    server.tool(tool.name, `${tool.description}\n\n[Paper: ${tool.paper}] [Backend: ${tool.backend}] [Regions: ${tool.bodyRegions.join(", ")}]`, paramShape, async (params) => {
...
// --- 健康检查资源 ---
server.resource("health", "health://status", async () => {
    const checks = {};
    // 检查各 backend
    for (const backend of ["gpu-local", "cpu-local"]) {
        try {
            const resp = await fetch(`${backend === "gpu-local" ? process.env.GPU_BACKEND_URL ?? "http://localhost:35677" : process.env.CPU_BACKEND_URL ?? "http://localhost:35678"}/health`, { signal: AbortSignal.timeout(5000) });
