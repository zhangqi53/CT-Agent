...
def run(**kwargs):
    ...
    if dataset[:4] == 'mayo':
        n_samples = len(list(__import__("pathlib").Path("dataset/mayo/test/Sin_CNN_64views").glob("data*.mat")))
    elif dataset[:4] == 'NBIA':
        n_samples = 40
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            ...
            import scipy.io as scio_save, os
            recon_dir = 'dataset/mayo/test/recon_64views'
            os.makedirs(recon_dir, exist_ok=True)
            scio_save.savemat(os.path.join(recon_dir, os.path.basename(file_name[0])), {'data': x_out})
